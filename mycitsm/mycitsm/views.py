...
ldappath = "ldap://8.8.8.8"#ldap服务器地址
...
values(%s, %s, '', '', 0);""",[hostname, hostname])
        cursor.execute("""insert into release_dbconfig(dbname, host, port, upload_dbname, active, EnvID) 
VALUES(%s, %s, %s, %s, '1', 4)""",[hostname, ip, port, hostname])
        cursor.execute("""insert into db_conninfo(dbname, envt, host, port, dbuser, password)
values(%s, 'DEV', %s, %s, %s, '')""", [hostname, ip, port, backupip])
        cursor.execute("""insert into sqltools_db_conninfo(dbid, dbname, host, port, dbuser, type) 
select id, dbname, %s, %s, %s, %s from release_dbconfig where dbname  = %s """, [ip, port, backupip, "W", hostname])
        cursor.execute("""insert into sqltools_db_conninfo(dbid, dbname, host, port, dbuser, type) 
select id, dbname, %s, %s, %s, %s from release_dbconfig where dbname  = %s """, [ip, port, backupip, "R", hostname])
